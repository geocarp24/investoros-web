...
async function loadContext(cfg) {
  ...
  const [persona, copy] = await Promise.all([
    safeRead(cfg?.persona_file ? join(__dirname, "..", cfg.persona_file) : join(ORACULO_DIR, "wi_homeowner_persona.md"), 4000),
...
// ─── Sonnet review call ───
async function reviewIdea(record, ctx, format = "Post", cfg = null) {
  ...
  // Tenant-aware prompt (2026-06-01). Falls back to Pinnacle defaults when cfg missing.
  const tenantName    = cfg?.tenant_name || "Pinnacle Holdings Group LLC";
  const tenantPhone   = cfg?.brand?.phone || cfg?.business?.phone || "(920) 777-9886";
  const tenantWebsite = cfg?.website?.replace(/^https?:\/\//, "") || "pinnaclegroupwi.com";
  const tenantSlug    = cfg?.tenant_id || "pinnacle";
  const personaCheck  = tenantSlug === "pinnacle"
    ? `Does the idea speak directly to one of the 6 Wisconsin distressed homeowner segments (foreclosure / inherited / divorce / back taxes / tired landlord / relocation)? Generic real estate content = REJECT.`
    : `Does the idea match the AUDIENCE PERSONA shown below? Generic content that doesn't fit ${tenantName}'s services + audience = REJECT.`;
  const brandVoiceRule = tenantSlug === "pinnacle"
    ? `Warm, no-pressure, no investor jargon, no salesy hype. "Cash buyer" OK. "ROI / cap rate / off-market deal" = REJECT.`
    : `Follows the brand voice defined in AUDIENCE PERSONA below. No pressure tactics, no vague price claims, no specific time guarantees ("in 24 hours"), no license #s.`;
  const complianceRule = tenantSlug === "pinnacle"
    ? `No FTC red flags ("guaranteed", "no risk"). No HUD Fair Housing violations (race/family/disability targeting). No promotion of homosexuality in visual concepts.`
    : `No FTC red flags ("guaranteed", "no risk"). Follow LEGAL/COMPLIANCE rules listed in AUDIENCE PERSONA below (especially NEVER_say items).`;

  const systemPrompt = `You are El Oráculo — quality gate for ${tenantName}'s social media pipeline. Your job: review one idea generated by El Social Media Manager and decide if El Creativo should render it.
...
Output ONLY a JSON object — no prose, no markdown fences:
{
  ...
  "rejection_category": "BRAND_VOICE_VIOLATION|COMPLIANCE_VIOLATION|PERSONA_MISMATCH|STRUCTURAL_INCOMPLETE|LANGUAGE_QUALITY|HOOK_WEAK|THEME_MISMATCH|NUMERIC_TIME_CLAIM|CTA_MISSING|OTHER — null if verdict=APPROVE"
}

REVIEW CRITERIA:
1. PERSONA FIT — ${personaCheck}
2. BRAND VOICE — ${brandVoiceRule}
3. COMPLIANCE — ${complianceRule}
4. HOOK QUALITY — Opens curiosity in <12 words. Should mention pain point or benefit.
5. CTA PRESENCE — phone ${tenantPhone} + ${tenantWebsite} mandatory in caption (this record is in ${lang} — perfect orthography for Spanish required: acentos á é í ó ú, ñ).
...
// ─── Process one record ───
async function processOne(record, ctx, tableId, format, cfg = null) {
  const f = record.fields || {};
  ...
  if (ANTHROPIC_KEY) {
    try {
      review = await reviewIdea(record, ctx, format, cfg);
    } catch (e) {
  ...
  const rejectionCategory = review.rejection_category || "OTHER";
  await smUpdate(tableId, record.id, {
    Status:             STATUS.RECHAZADA,
    Oraculo_Score:      score,
    Oraculo_Notes:      `src=${reviewSource}`,
    Error_Reason:       reason,
    Rejection_Category: rejectionCategory,  // Learning loop (CC 2026-06-04)
  });

  // Learning loop: upsert rejection count in Geo_Lessons (tbl2VhJyx4KJIqNqL)
  const GEO_LESSONS_TABLE = "tbl2VhJyx4KJIqNqL";
  const tenantId = cfg?.tenant_id || "geo-carpentry";
  try {
    const lFilter = encodeURIComponent(`AND({category}='${rejectionCategory}',{tenant_id}='${tenantId}')`);
    const lResp = await smFetch(GEO_LESSONS_TABLE, `filterByFormula=${lFilter}&maxRecords=1`);
    const existing = lResp.records?.[0];
    if (existing) {
      await smUpdate(GEO_LESSONS_TABLE, existing.id, {
        rejection_count: (existing.fields.rejection_count || 0) + 1,
        last_seen_at:    new Date().toISOString(),
        rule:            existing.fields.rule || review.improvement_notes || "",
      });
    } else {
      await fetch(`https://api.airtable.com/v0/${SM_BASE}/${GEO_LESSONS_TABLE}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${SM_TOKEN}`, "Content-Type": "application/json" },
        body: JSON.stringify({ fields: {
          title:           `L-${rejectionCategory} auto`,
          category:        rejectionCategory,
          rule:            review.improvement_notes || "",
          bad_example:     "",
          good_example:    "",
          rejection_count: 1,
    ...
    console.error(`[oraculo] geo_lessons upsert failed: ${e.message}`);
  }
...
async function main() {
  ...
  const ctx = await loadContext(cfg);
  ...
  for (const item of pending) {
    ...
    let out;
    try { out = await processOne(rec, ctx, tableId, format, cfg); }
    catch (e) {
